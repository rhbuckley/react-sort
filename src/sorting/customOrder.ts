...
export const customOrder = (order: SortingOrder, size: number): number[] => {
    ...
    if (order === "sorted-desc")
        return new Array(size).fill(0).map((_, i) => size - i);

    // normal
    if (order === "normal")
        return generateNormalDistribution(size / 2, size / 4, size);

    // normal-skew-left
    if (order === "normal-skew-left")
        return generateNormalDistribution(size / 4, size / 2, size, 1.2);

    // normal-skew-right
    if (order === "normal-skew-right")
        return generateNormalDistribution(size / 4, size / 2, size, -1.2);
