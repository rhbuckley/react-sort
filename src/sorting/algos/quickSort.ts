...
export class QuickSort extends SortingAlgorithm {
    ...
    private partition(start: number, end: number): number {
        ...
        this.colors[end] = "var(--color-bar-swap-1)"; // Color the pivot
        ...
        for (let j = start; j < end; j++) {
            ...
            this.colors[j] = "var(--color-bar-accent)"; // Color elements being compared with the pivot
            this.saveStep();
        }

        this.swap(i, end); // Move pivot to its sorted position
        this.colors[i] = "var(--color-bar-swap-1)"; // Color the pivot at its sorted position
        this.saveStep();
